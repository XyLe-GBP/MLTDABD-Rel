...
{
    public partial class FormIVAssetDownload : Form
    {

        static FormIVAssetDownload()
        {
            CheckForIllegalCrossThreadCalls = false;
        }

        private FormIVAssetDownload()
        {
            InitializeComponent();
        }
        ...
        public static DialogResult ShowDownload(IReadOnlyList<AssetInfo> assetInfoList, IWin32Window owner)
        {
            ...
            using (var f = new FormIVAssetDownload())
            {
...
#pragma warning restore SYSLIB0014 // 型またはメンバーが旧型式です
        ...
        private static readonly Regex AssetVersionPattern = new(@"^\d+$");
        private static readonly Regex UnityHeaderVersionPattern = new(@"^\d+\.\d+\.\d+[fp]\d+$");
        private static readonly Regex UnityAssetVersionPattern = new(@"^\d+\.\d+$");
